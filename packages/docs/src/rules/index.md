...
2 人のプレイヤーはそれぞれ 4x4 のマスからなるボードと 0 のスコアを持っています。
...
直前のメインフェーズでマージが発生した回数が X のとき、1 個以上の、総和が 2<sup>X+1</sup> の正整数を置きます。
ただし、存在しないマス、および、既に正整数が置かれたマスは選べません。また、各正整数は 2 以上の 2 の累乗数でなければなりません。
...
1. 対戦相手のプレイヤーのターンに、以下の条件のいずれかを満たした場合
   ...
   - 対戦相手の AI プログラムの出力が、定められたフォーマットもしくは制約を満たさなかった場合
     - 例えば、アタックフェーズで既に正整数が置かれているマスを選んだ場合も該当します。
1. 2 人のプレイヤーがそれぞれ 1000 ターン行動した後、自分のスコアが対戦相手のスコアより高かった場合
1. 2 人のプレイヤーがそれぞれ 1000 ターン行動した後、自分のスコアと対戦相手のスコアが等しく、かつ、自分が後攻であった場合
